...
def invoke_llm(model, system_prompt, user_prompt, max_tokens, temperature, top_p, index=-1,base_index_list_index=0):
    if model == "gpt4o":
        ...
        return ollama.ollama(user_prompt, system_prompt, model, max_tokens=max_tokens, temperature=temperature, top_p=top_p, index=index, base_index_list_index=base_index_list_index)
            ...
            if key in current:
                ...
                if closest_matches:
                    ...
                    for match in closest_matches:
                        corrected_references.append(match)
                    print(f"Corrected reference: {ref} to {closest_matches}")
                else:
                    print(f"Missing key in reference: {ref}")
                found = False
                break
        
        if found:
            # If we completed the traversal, add the reference
