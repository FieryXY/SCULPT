...
from sklearn.metrics import f1_score, classification_report, accuracy_score, cohen_kappa_score, precision_recall_fscore_support
import utils
import re
import pandas as pd
import numpy as np
...
def process_example(ex, predictor, prompt, task, index, max_servers=40):
    i = index % max_servers
    pred, response, token_usage = predictor.inference(ex, prompt, task, i)
    return ex, pred, response, token_usage

def calculate_regression_metrics(labels, preds, metric="inverse_mae"):
    labels = np.array(labels)
    preds = np.array(preds)

    if metric == "inverse_mae":
...
def calculate_harmonic_mean(score_list):
    harmonic_mean = len(score_list) / sum(1/max(x, 0.001) for x in score_list)
    average = sum(x for x in score_list) /len(score_list)
    return (harmonic_mean + average)/2
...
class MultiLabelTask(DataProcessor):
    ...
    def run_evaluate(self, predictor, prompt, test_exs, n=100, taskName=None):
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=self.max_threads) as executor:
            futures = [executor.submit(process_example, ex, predictor, prompt.prompt, taskName, i) for i, ex in enumerate(test_exs[:n])]
        ...
        # Add overall metrics
        allMetrics["macro avg"] = {"f1-score": macro_f1}
        allMetrics["weighted avg"] = {"f1-score": weighted_f1}
        ...
        return exact_match_accuracy, texts, labels, preds, responses, allMetrics, token_usage, promptTokenCount
...
class RegressionTask(DataProcessor):
    ...
    @abstractmethod
    def get_regression_scores(self, label):
        pass
    ...
    def run_evaluate(self, predictor, prompt, test_exs, n=100, taskName=None):
        ...
        gt_scores = [self.get_regression_scores(str(label)) for label in labels]
        pred_scores = [self.get_regression_scores(str(pred)) for pred in preds]
        allMetrics = {}
        for key in gt_scores[0].keys():
            score = calculate_regression_metrics([score[key] for score in gt_scores], [score[key] for score in pred_scores], metric=self.metric)
            allMetrics[key] = score
        
        # Harmonic mean of various scores
        harmonic_mean = calculate_harmonic_mean(list(allMetrics.values()))
        promptTokenCount = promptTokenCount / count
        return harmonic_mean, texts, labels, preds, responses, allMetrics, token_usage, promptTokenCount
...
class ClassificationTask(DataProcessor):
    ...
    def run_evaluate(self, predictor, prompt, test_exs, n=100, taskName=None):
        ...
        if taskName == "bbh-disambiguation-qa" or taskName == "bbh-salient-translation":
...
class BinaryClassificationTask(ClassificationTask):
    ...
    categories_UHRS_Topical = {14:"pornography", 1:"watch movies, streaming, mp4, watch tv, watch series", 2:"torrents", 3:"betting, gambling, casino", 4:"anime", 5:"songs, mp3", 6:"play online games, including game downloads", 7:"read or download online books", 8:"software download, excluding game downloads", 9:"pretends to provide technical support", 10:"crosswords", 11:"exam and tests", 26:"dealership", 12:"cars and automobiles", 13:"jobs and careers", 15:"online radio station", 16:"pharmaceuticals", 17:"medicine and healthcare", 18:"sport", 19:"real estate", 33:"coupons", 20:"online shop", 21:"newspaper", 22:"articles about books or literature", 23:"articles or blogs about movies, tv shows", 24:"learning courses", 25:"educational", 27:"obituary", 28:"finances, loans", 29:"food", 30:"travel", 31:"religion", 32:"IT", 60:"other business", 61:"information, knowledge", 0:"other"}
    ...
    def stringify_prediction(self, pred, taskName):
        ...
        elif taskName == "bbh-disambiguation-qa":
...
class BBHFormalFallaciesTask(BinaryClassificationTask):
    ...
    def get_train_examples(self):
        exs=[]
        file = open("data/formal_fallacies/train.tsv", "r", encoding="utf-8")
        i = 0
        for line in file:
            ...
            if category.lower().strip() == "valid":
                ...
                label = 1
            exs.append({'id': f'train-{i}', 'label': int(label), 'text': text})
            i+=1

        file.close()
        file = open("data/formal_fallacies/validation.tsv", "r", encoding="utf-8")
        i = 0
        for line in file:
            text, category = line.split("\t")
            if category.lower().strip() == "valid":
    ...
    def get_test_examples(self):
        exs=[]
        file = open("data/formal_fallacies/test.tsv", "r", encoding="utf-8")
        i = 0
        for line in file:
            text, category = line.split("\t")
            if category.lower().strip() == "valid":
                ...
                label = 1
            exs.append({'id': f'test-{i}', 'label': int(label), 'text': text})
            i+=1
...
class BBHDisambiguationQATask(BinaryClassificationTask):
    ...
    def get_train_examples(self):
        exs=[]
        file = open("data/disambiguation_qa/train.tsv", "r", encoding="utf-8")
        i = 0
        for line in file:
            text = ""
            while "\t" not in line:
        ...
        file.close()
        file = open("data/disambiguation_qa/validation.tsv", "r", encoding="utf-8")
        i = 0
        for line in file:
            text = ""
            while "\t" not in line:
    ...
    def get_test_examples(self):
        exs=[]
        file = open("data/disambiguation_qa/test.tsv", "r", encoding="utf-8")
        i = 0
        for line in file:
            text = ""
            while "\t" not in line:
...
class BBHSalientTranslationTask(BinaryClassificationTask):
    ...
    def get_train_examples(self):
        exs=[]
        file = open("data/salient_translation/train.tsv", "r", encoding="utf-8")
        i = 0
        for line in file:
            text = ""
            while "\t" not in line:
        ...
        file.close()
        file = open("data/salient_translation/validation.tsv", "r", encoding="utf-8")
        i = 0
        for line in file:
            text = ""
            while "\t" not in line:
    ...
    def get_test_examples(self):
        exs=[]
        file = open("data/salient_translation/test.tsv", "r", encoding="utf-8")
        i = 0
        for line in file:
            text = ""
            while "\t" not in line:
...
class BBHCausalJudgementTask(BinaryClassificationTask):
    ...
    def get_train_examples(self):
        exs=[]
        file = open("data/causal_judgement/train.tsv", "r", encoding="utf-8")
        i = 0
        for line in file:
            text, category = line.split("\t")
        ...
        file = open(r"data/causal_judgement/validation.tsv", "r", encoding="utf-8")
        i = 0
        for line in file:
            text, category = line.split("\t")
            if category.lower().strip() == "no":
    ...
    def get_test_examples(self):
        exs=[]
        file = open("data/causal_judgement/test.tsv", "r", encoding="utf-8")
        i = 0
        for line in file:
            text, category = line.split("\t")
            if category.lower().strip() == "no":
