...
def run_ollama_chat_http(model_name, messages, options):
    ...
    # Concatenate the first system prompt and first user prompt with a newlines in between
    first_system_prompt = ""
    first_user_prompt = ""
    for message in messages:
        if message["role"] == "system":
            first_system_prompt = message["content"]
            break
    for message in messages:
        if message["role"] == "user":
            first_user_prompt = message["content"]
            break
    generate_endpoint_prompt = "\n".join([first_system_prompt, first_user_prompt])
    ...
    try:
        ...
        response = ollama_client.generate(
            model=model_name,
            prompt=generate_endpoint_prompt,
        )

        # Extract the content and token details directly from the ollama response
        ...
        print(f"An error occurred with the ollama request: {e}")
        return "Could not get a response", {"prompt_tokens": 0, "completion_tokens": 0}


def ollama(messages, model="gpt4o", history=None, max_tokens=256, temperature=0, top_p=1.0, frequency_penalty=0, presence_penalty=0, retry=5, index=-1):
    ...
    #     }
    # ]
    messages = history + messages if history else messages
    # Find the user message from messages
    user_message = ""
    for message in messages:
        if message["role"] == "user":
    ...
    while(flag):
        try:
            ...
            gen_response, token_details = run_ollama_chat_http(model_name, messages, options)
            flag = False
        except Exception as e:
            print(e)
    ...
    for i in range(num_base_urls):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for prompt in prompts[start:end]:
                ...
                futures.append(executor.submit(ollama, messages, model=model, max_tokens=max_tokens, temperature=temperature, top_p=top_p, frequency_penalty=frequency_penalty, presence_penalty=presence_penalty, retry=retry, index=i))
